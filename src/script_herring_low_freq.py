...
def createSettings(fish_length, depth, incidence_angle):
    ...
    b_0 = (0.01 + (fish_length - 0.1) * 0.05) * 0.5  # b_0=0.01*0.5 is from Gorska & Ona - now varying so that b_0 is 0.01 for a 30 cm fish
...
if __name__ == '__main__':
    ...
    incidence_angles = [90, 88, 86, 84, 82, 80]   # [90, 82, 75]
    depths = [10, 50, 100]

    for incidence_angle in incidence_angles:
        for fish_length in fish_lengths:
            for depth in depths:
                print('Running with length {}, incidence angle {}, and depth {}'.format(fish_length, incidence_angle,
                                                                                        depth))
                ...
                # c_w = 1500               # sound speed in water
                # c_s = 0.23 * c_w         # the sound speed is assumed not to depend on depth
                # herring_settings = HerringSettings('herring', ro_s, c_s, 0.04121049493551254, 0.006071448253824418,
                #                                    1000, 1000, 260000, incidence_angle)
                ### end special settings
                #solver = BiCGSTABSolver('ILU')
                solver = IterativeRefinement('LU')
                ts_file_name = 'ts_vs_freq_loop_{}_a_{:.5f}_b_{:.5f}_f1_{}_f2_{}_rhos_{:.2f}_IncAngle_{}_depth_{}_length_{}_{}.csv'.format(
                    herring_settings.prefix,
                ...
                freq_resp_file = os.path.join(ParentDIR, 'model_results\model_low_freq_b0_varies_results', ts_file_name)
