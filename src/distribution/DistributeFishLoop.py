...
def main():
    ...
    database_dir = os.path.join(ParentDIR, 'model_results','model_low_freq_results/')
    ...
    N = 10000
    a, b = 4.0, 10.0  # spheroid axes

    min_dist = 0.1
    Average_school_Depth = 100 # m
    ...
    for i in range(num_realizations):
        ...
        for ii in range(0, len(points)):
            ...
            if length_string == 'mix':
                Length = 0.1 if ii < len(points)//2 else 0.3# m.  This can be changed to include distribution of ranges
            elif length_string == '0.1':
                Length = 0.1
            elif length_string == '0.3':
                Length = 0.3
            else:
                raise ValueError('Unknown length string {}'.format(length_string))
            ...
            #plt.plot(freq, 20*np.log10(np.abs(f_bs_ii)), color='k', linestyle='--', linewidth = 1)

        p_far /= np.sqrt(len(points))
        Total_p = 20*np.log10(np.abs(p_far))


        def save_realization():
            output_dir = os.path.join(ParentDIR, 'model_results', 'school_results', 'realizations')
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            ouput_file_name = 'sv_school_S_{}_depth_{}_size_{}_realiz_{}.csv'.format(N, Average_school_Depth, length_string, i)
            output_file = os.path.join(output_dir, ouput_file_name)
            output_data = pd.DataFrame({'Freq_kHz': freq, 'sv': p_far, 'Sv': Total_p})
            output_data.to_csv(output_file, encoding='utf-8', sep=',', index=False, header=True)

        #save Total_p to csv-file, run several times
        save_realization()
        print('realization {} saved'.format(i))
